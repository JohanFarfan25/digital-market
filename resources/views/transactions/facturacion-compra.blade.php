                    ...
                    <form id="checkout-form">
                            ...
                            <div class="col-md-6">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="supplier" class="form-control-label">{{ __('Proveedor') }}</label>
                                        <div class="@error('supplier')border border-danger rounded-3 @enderror">
                        ...
                        <div class="row mt-3">
                            <div class="col-md-12 text-end">
                                ...
                                <button id="save" class="btn btn-success" disabled>Generar orden de compra</button>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#supplier_id').on('change', function() {
                ...
                let name = selectedOption.data('name') || 'No disponible';
                let email = selectedOption.data('email') || 'No disponible';
                let phone = selectedOption.data('phone') || 'No disponible';

                $("#selected_supplie_details").html(`
            ...
            $('#search_supplier').on('input', function() {
                ...
                if (query === '') {
                    $('#supplier_id').empty(); // Limpiar el select
                    $('#supplier_list').html(
                        '<p class="text-muted">Proveedores...</p>'); // Restaurar mensaje de proveedor
                    $('#supplier_id').empty().append('<option value="">Seleccione un proveedor</option>');
                    $("#selected_supplie_details").empty();
                    $('#selected_supplie_details').html(
                        ' <p class="text-muted">No hay proveedor seleccionado</p>');
                }
            });
            ...
            $(document).ready(function() {
                $('#search_supplier').on('input', function() {
                    ...
                    if (query === '') {
                        $('#supplier_list').show(); // Mostrar si se borra la búsqueda
                ...
                $('#search_supplier').on('keyup', function() {
                    ...
                    if (query.length > 1) {
                        list.html('<p class="text-muted">Cargando...</p>');
                        $.ajax({
                            url: '/supplier-search',
                            method: 'GET',
                            data: {
                            ...
                            success: function(response) {
                                ...
                                response.forEach(function(supplier) {
                                    list.append(`
                                        <div class="list-group-item list-group-item-action supplier-item border p-2 mb-1 rounded bg-light"
                                            data-id="${supplier.id}"
                                ...
                                // Evento para seleccionar un cliente al hacer clic
                                $('.supplier-item').on('click', function() {
                                    ...
                                    // Ocultar la lista de proveedores
                                    $('#supplier_list').hide();
                                });
                            }
                        });
